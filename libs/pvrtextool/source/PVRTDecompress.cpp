                      ...
                      int nMode)
{
	...
	if (x < 4 || y < 4)
	{
		...
		char* pTempBuffer = (char*)malloc((x > 4 ? x : 4) * (y > 4 ? y : 4) * 4);
		i32read = ETCTextureDecompress(pSrcData, (x > 4 ? x : 4), (y > 4 ? y : 4),
		                               pTempBuffer, nMode);

		for (unsigned int i = 0; i < y; i++)
		{
			// copy from larger temp buffer to output data
			memcpy((char*)(pDestData) + i * x * 4, pTempBuffer + (x > 4 ? x : 4) * 4 * i, x * 4);
		}
