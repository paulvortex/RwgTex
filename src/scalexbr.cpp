...
ScalerCfg DefaultScalerCfg = { 1, 30, 3.6, 2.2, 0, false, false, false };
...
template <unsigned int N, unsigned int M> inline void alphaBlend(uint32_t& dst, uint32_t col, const ScalerCfg& cfg) // blend color over destination with opacity N / M
{
	...
	dst =  (redMask   & static_cast<uint32_t>(floor((col & redMask  ) * alpha + 0.49f) + floor((dst & redMask  ) * (1 - alpha) + 0.49f))) | // this works because 8 upper bits are free
           (greenMask & static_cast<uint32_t>(floor((col & greenMask) * alpha + 0.49f) + floor((dst & greenMask) * (1 - alpha) + 0.49f))) |
           (blueMask  & static_cast<uint32_t>(floor((col & blueMask ) * alpha + 0.49f) + floor((dst & blueMask ) * (1 - alpha) + 0.49f)));
}
...
/*
inline void rgbtoLuv(uint32_t c, double& L, double& u, double& v)
{
...
/*
inline void rgbtoHsl(uint32_t c, double& h, double& s, double& l)
{
...
inline double distHSL(uint32_t pix1, uint32_t pix2, double lightningWeight)
{
    double h1 = 0;
    double s1 = 0;
    double l1 = 0;
...
inline double distNonLinearRGB(uint32_t pix1, uint32_t pix2)
{
...
inline double distYUV(uint32_t pix1, uint32_t pix2, double luminanceWeight)
{
...
inline double colorDist(uint32_t pix1, uint32_t pix2, const ScalerCfg& cfg)
{
    ...
    return distYCbCr(pix1, pix2, cfg.luminanceWeight_);
}

inline bool equalColor(uint32_t col1, uint32_t col2, const ScalerCfg& cfg)
{
	return colorDist(col1, col2, cfg) < cfg.equalColorTolerance_;
}
...
-----------------
*/
FORCE_INLINE BlendResult preProcessCorners(const Kernel_4x4& ker, const xbrz::ScalerCfg& cfg) //result: F, G, J, K corners of "GradientType"
{
    BlendResult result = {};
	...
	// transform
	#define dist(col1,col2) colorDist(col1, col2, cfg)
...
template <class Scaler, RotationDegree rotDeg> FORCE_INLINE void scalePixel(const Kernel_3x3& ker, uint32_t* target, int trgWidth, unsigned char blendInfo, const xbrz::ScalerCfg& cfg)
{
    ...
    {
		// transform
		#define eq(col1,col2) (equalColor(col1, col2, cfg))
		#define dist(col1,col2) (colorDist(col1, col2, cfg))
        ...
        if (doLineBlend)
        {
            ...
            {
                if (haveSteepLine)
                    Scaler::blendLineSteepAndShallow(px, out, cfg);
                else
                    Scaler::blendLineShallow(px, out, cfg);
            }
            else
            {
                if (haveSteepLine)
                    Scaler::blendLineSteep(px, out, cfg);
                else
                    Scaler::blendLineDiagonal(px, out, cfg);
            }
        }
        else
            Scaler::blendCorner(px, out, cfg);
...
template <class Scaler> void scaleImage(const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight, const xbrz::ScalerCfg& cfg, int yFirst, int yLast)
{
...
struct Scaler2x
{
    ...
    template <class OutputMatrix> static void blendLineShallow(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 4>(out.template ref<scale - 1, 0>(), col, cfg);
        alphaBlend<3, 4>(out.template ref<scale - 1, 1>(), col, cfg);
    }

    template <class OutputMatrix> static void blendLineSteep(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 4>(out.template ref<0, scale - 1>(), col, cfg);
        alphaBlend<3, 4>(out.template ref<1, scale - 1>(), col, cfg);
    }

    template <class OutputMatrix> static void blendLineSteepAndShallow(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 4>(out.template ref<1, 0>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<0, 1>(), col, cfg);
        alphaBlend<5, 6>(out.template ref<1, 1>(), col, cfg); //[!] fixes 7/8 used in xBR
    }

    template <class OutputMatrix> static void blendLineDiagonal(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 2>(out.template ref<1, 1>(), col, cfg);
    }

    template <class OutputMatrix> static void blendCorner(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        //model a round corner
        alphaBlend<21, 100>(out.template ref<1, 1>(), col, cfg); //exact: 1 - pi/4 = 0.2146018366
    }
};
...
struct Scaler3x
{
    ...
    {
        alphaBlend<1, 4>(out.template ref<scale - 1, 0>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<scale - 2, 2>(), col, cfg);
        alphaBlend<3, 4>(out.template ref<scale - 1, 1>(), col, cfg);
    ...
    {
        alphaBlend<1, 4>(out.template ref<0, scale - 1>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<2, scale - 2>(), col, cfg);
        alphaBlend<3, 4>(out.template ref<1, scale - 1>(), col, cfg);
    ...
    template <class OutputMatrix> static void blendLineSteepAndShallow(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 4>(out.template ref<2, 0>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<0, 2>(), col, cfg);
        alphaBlend<3, 4>(out.template ref<2, 1>(), col, cfg);
        alphaBlend<3, 4>(out.template ref<1, 2>(), col, cfg);
        out.template ref<2, 2>() = col;
    }

    template <class OutputMatrix> static void blendLineDiagonal(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 8>(out.template ref<1, 2>(), col, cfg);
        alphaBlend<1, 8>(out.template ref<2, 1>(), col, cfg);
        alphaBlend<7, 8>(out.template ref<2, 2>(), col, cfg);
    }

    template <class OutputMatrix> static void blendCorner(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        //model a round corner
        alphaBlend<45, 100>(out.template ref<2, 2>(), col, cfg); //exact: 0.4545939598
...
struct Scaler4x
{
    ...
    template <class OutputMatrix> static void blendLineShallow(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        ...
        alphaBlend<3, 4>(out.template ref<scale - 1, 1>(), col, cfg);
        alphaBlend<3, 4>(out.template ref<scale - 2, 3>(), col, cfg);
    ...
    template <class OutputMatrix> static void blendLineSteep(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        ...
        alphaBlend<3, 4>(out.template ref<1, scale - 1>(), col, cfg);
        alphaBlend<3, 4>(out.template ref<3, scale - 2>(), col, cfg);
    ...
    template <class OutputMatrix> static void blendLineSteepAndShallow(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<3, 4>(out.template ref<3, 1>(), col, cfg);
        alphaBlend<3, 4>(out.template ref<1, 3>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<3, 0>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<0, 3>(), col, cfg);
        alphaBlend<1, 3>(out.template ref<2, 2>(), col, cfg); //[!] fixes 1/4 used in xBR
        out.template ref<3, 3>() = out.template ref<3, 2>() = out.template ref<2, 3>() = col;
    }

    template <class OutputMatrix> static void blendLineDiagonal(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 2>(out.template ref<scale - 1, scale / 2    >(), col, cfg);
        alphaBlend<1, 2>(out.template ref<scale - 2, scale / 2 + 1>(), col, cfg);
    ...
    {
        //model a round corner
        alphaBlend<68, 100>(out.template ref<3, 3>(), col, cfg); //exact: 0.6848532563
        alphaBlend< 9, 100>(out.template ref<3, 2>(), col, cfg); //0.08677704501
        alphaBlend< 9, 100>(out.template ref<2, 3>(), col, cfg); //0.08677704501
    }
};
...
struct Scaler5x
{
    ...
    template <class OutputMatrix> static void blendLineShallow(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 4>(out.template ref<scale - 1, 0>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<scale - 2, 2>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<scale - 3, 4>(), col, cfg);
    ...
    template <class OutputMatrix> static void blendLineSteep(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 4>(out.template ref<0, scale - 1>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<2, scale - 2>(), col, cfg);
        alphaBlend<1, 4>(out.template ref<4, scale - 3>(), col, cfg);
    ...
    template <class OutputMatrix> static void blendLineSteepAndShallow(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        ...
        alphaBlend<2, 3>(out.template ref<3, 3>(), col, cfg);
    }

    template <class OutputMatrix> static void blendLineDiagonal(uint32_t col, OutputMatrix& out, const ScalerCfg& cfg)
    {
        alphaBlend<1, 8>(out.template ref<scale - 1, scale / 2    >(), col, cfg);
        alphaBlend<1, 8>(out.template ref<scale - 2, scale / 2 + 1>(), col, cfg);
        alphaBlend<1, 8>(out.template ref<scale - 3, scale / 2 + 2>(), col, cfg);

        alphaBlend<7, 8>(out.template ref<4, 3>(), col, cfg);
        alphaBlend<7, 8>(out.template ref<3, 4>(), col, cfg);
    ...
    {
        //model a round corner
        alphaBlend<86, 100>(out.template ref<4, 4>(), col, cfg); //exact: 0.8631434088
        alphaBlend<23, 100>(out.template ref<4, 3>(), col, cfg); //0.2306749731
        alphaBlend<23, 100>(out.template ref<3, 4>(), col, cfg); //0.2306749731
...
void scale(size_t factor, const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight, const xbrz::ScalerCfg& cfg, int yFirst, int yLast)
{
    switch (factor)
    {
        case 2:
